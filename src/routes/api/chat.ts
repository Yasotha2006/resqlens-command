...
const readToken = (request: Request) =>
  request.headers.get("cookie")?.match(/(?:^|; )resqlens_session=([^;]+)/)?.[1];
...
async function session(request: Request) {
  ...
  const { data } = await supabaseAdmin
    .from("resqlens_copilot_sessions")
    .select("id,messages")
  ...
  const created = await supabaseAdmin
    .from("resqlens_copilot_sessions")
    .insert({ access_token_hash: tokenHash, messages: [] })
    .select("id,messages")
    .single();
    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        return Response.json(
          ...
          current.fresh ? { headers: { "Set-Cookie": cookieHeader(current.token) } } : {},
        );
      },
      PUT: async ({ request }) => {
